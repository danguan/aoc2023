...
class Solution(object):
    ...
    def extrapolate_value(self, seq: List[int]) -> int:
        """Extrapolates the prev value in seq based on the sequence diffs.

        When finding the previous value in each intermediate diff sequence,
        append the "previous value" at end of the diff list.
        """
        diffs = [seq]
        ...
        # 0 is the "previous value" of diffs[-1]
        diffs[-1].append(0)

        for diff_idx in range(len(diffs) - 2, -1, -1):
            # Use the next diff's latest value to find the new prev value.
            diffs[diff_idx].append(diffs[diff_idx][0] - diffs[diff_idx + 1][-1])
