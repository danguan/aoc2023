...
class Solution(object):
    ...
    def find_horiz_value(self, pattern: List[str]) -> int:
        ...
        def horiz_mirror_diffs(row: int, mirror_row: int, diffs: int = 0) -> int:
            ...
            return horiz_mirror_diffs(row - 1, mirror_row + 1, diffs)

        # Check each starting row for possible mirrored rows
        for r in range(len(pattern) - 1):
            if horiz_mirror_diffs(r, r + 1) == 1:
                return (r + 1) * 100
    ...
    def find_vert_value(self, pattern: List[str]) -> int:
        ...
        def vert_mirror_diffs(col: int, mirror_col: int, diffs: int = 0) -> int:
